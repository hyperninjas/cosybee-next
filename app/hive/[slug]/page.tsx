...
export default async function HiveArticlePage({
  ...
  return (
    <main className="flex-1">
      <article className="mx-auto max-w-[800px] px-6 pt-10 pb-16 sm:px-10 lg:pt-14 lg:pb-20">
        {/* back link */}
        <Link
          href="/hive"
          className="inline-flex items-center gap-2 rounded-full border border-[#EE3D1A] px-4 py-2 text-sm font-medium text-[#EE3D1A] transition-colors hover:bg-[#FFF5F2]"
        >
          <BackArrow />
          Back to Blog
        </Link>
          ...
          <div className="mt-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="text-sm">
                ...
                <div className="text-[#545454]">{article.author.date}</div>
              </div>
            </div>
            <button
              type="button"
              aria-label="Share article"
              className="flex h-10 w-10 items-center justify-center rounded-full border border-neutral-200 bg-white text-black transition-colors hover:bg-neutral-50"
            >
              <ShareIcon />
            </button>
          </div>
        ...
        {/* hero image */}
        <div className="relative mt-8 aspect-[4/3] overflow-hidden rounded-3xl sm:aspect-[16/10]">
          <Image
            src={article.image}
        ...
        {/* body */}
        <div className="mt-10 space-y-10 text-[#545454]">
      ...
      {/* more blogs */}
      {moreBlogs.length > 0 && (
        <section className="mx-auto max-w-[800px] px-6 pb-16 sm:px-10 lg:pb-24">
          <h2 className="text-2xl font-extrabold text-black sm:text-3xl">
            More blogs
          </h2>
          <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
            {moreBlogs.map((a) => (
              <MoreBlogsCard key={a.slug} a={a} />
            ))}
          </div>
        </section>
      )}
