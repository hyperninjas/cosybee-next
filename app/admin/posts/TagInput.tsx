    ...
    <div className="relative">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {tags.map((t, i) => (
          <span
            key={t}
            className="inline-flex items-center gap-1 rounded-md bg-[#F3F3F3] px-2 py-1 text-sm font-medium text-[#444]"
          >
            #{t}
            <button
              type="button"
              onClick={() => removeAt(i)}
              aria-label={`Remove ${t}`}
              className="leading-none text-[#999] hover:text-[#B4332A]"
            >
              ×
            </button>
          </span>
      ...
      {showDropdown && (
        <ul className="absolute z-10 mt-1 max-h-56 w-full max-w-xs overflow-auto rounded-lg border border-[#E2E2E2] bg-white py-1 shadow-lg">
          {matches.map((s) => (
            <li key={s}>
              <button
                type="button"
