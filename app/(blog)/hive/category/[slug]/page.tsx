...
/**
 * Prerender a landing page for every category a published Hive article is
...
export async function generateMetadata({
  params,
}: PageProps<"/hive/category/[slug]">): Promise<Metadata> {
  ...
  return pageMetadata({
    ...
    description: `${match.label} articles on ${LABEL} — guides, stories and advice from EnergieBee.`,
    ogDescription: `${match.label} articles on ${LABEL}.`,
...
export default async function HiveCategoryPage({
  params,
}: PageProps<"/hive/category/[slug]">) {
  const { slug } = await params;
